...
def meanrate(arr):
  ...
  # No weighting necessary if there's only one bin in the radial direction.
  elif arr.shape[0]==1:
    warr = arr
  else:
...
def plotevent(ev):
  ...
  scomplex = [ ev.sfft(m) for m in modes[:2] ]
  ...
  PW[2, 1].setParams (text=notex('Not Applicable'), yticklabels=[], ylabel='' )
